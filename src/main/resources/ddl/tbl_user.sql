...
comment on column tbl_user.department_key is '부서 키';

comment on column tbl_user.position_key is '직위 키';

comment on column tbl_user.role_key is '직책 및 권한 키';

comment on column tbl_user.user_image_key is '유저 이미지 키';

comment on column tbl_user.stamp_image_key is '날인 이미지 키';

comment on column tbl_user.token_value is 'JWT 토큰 값';
